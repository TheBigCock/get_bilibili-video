...
def single_download(origin_video_url, acc_quality,num):
    '''单个视频实现下载'''
    # 请求视频链接，获取信息
    ...
    # listinform = str(html.xpath('//head/script[3]/text()')[0])
    # listjson = json.loads(listinform)
    # name = listjson['videoData']['pages'][num]['part']

    # # 获取文件夹的名称(获取标题)
    # dirname = html.xpath('//*[@id="viewbox_report"]/h1/span/text()')[0].encode('ISO-8859-1').decode('utf-8')
    dirname = "2021"  #或者自定义
    if not os.path.exists(dirname):
        ...
        os.makedirs(dirname)
        print('目录文件创建成功!')
    title = os.getcwd() + '\\' + dirname + '\\' + str(num)
...
def download_video_single(referer_url, video_url, audio_url, video_name):
    ...
    # CombineVideoAudio(video_name)
    CombineVideoAudio(video_name + '_video.mp4',video_name + '_audio.mp4',video_name + '_output.mp4')

def CombineVideoAudio(videopath,audiopath,outpath):
    print("视频合成开始：%s" % videopath)
    subprocess.call(("ffmpeg -i " + videopath + " -i " + audiopath + " -c copy " + outpath).encode(
                "utf-8").decode("utf-8"), shell=True)

    os.remove(videopath)
    os.remove(audiopath)
    print("视频合成结束：%s" % outpath)
...
if __name__ == '__main__':
    ...
    if int(range_start)<=int(range_end):
        for i in range(int(range_start),int(range_end)+1):
            single_download(url+'?p='+str(i),0,i)
